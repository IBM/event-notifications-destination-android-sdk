...
public class ENPush extends FirebaseMessagingService{
  ...
  public final static String REGION_BNPP = "eu-es";
  ...
  */
  public void registerDeviceWithUserId(String userId, ENPushResponseListener<String> listener) {
  ...
  *                 method is called otherwise
  */
  public void registerDevice(ENPushResponseListener<String> listener) {
  ...
  */
  public void subscribe(final String tagName, final ENPushResponseListener<String> listener) {
    if (isAbleToSubscribe()) {
      ...
      listener.onFailure(new ENPushException("ENPush: Error while subscribing"));
    }
  }
  ...
  */
  ...
  final ENPushResponseListener<String> listener) {
    if (isAbleToSubscribe()) {
      ...
      listener.onFailure(new ENPushException("ENPush: Error while unsubscribing"));
    }
  }
  ...
  */
  public void unregister(final ENPushResponseListener<String> listener) {
    ...
    if (this.deviceId == null) {
      this.deviceId = ENPushUtils.getContentFromSharedPreferences(appContext, instanceId+ destinationId + ENPushConstants.DEVICE_ID);
    }
...
//  }
  ...
  private void registerInBackground(final String userId) {
    ...
    FirebaseMessaging.getInstance().getToken().addOnCompleteListener(task -> {
      if (!task.isSuccessful()) {
        ...
        registerResponseListener.onFailure((new ENPushException("Unable to retrieve deviceToken after maximum retries")));
        return;
      }
      deviceToken = task.getResult();
      logger.info("ENPush:registerInBackground() - Successfully registered with FCM. Returned deviceToken is: " + deviceToken);
      computeRegId();
  ...
  private boolean registerWithUserId(final String userId) {
    ...
    if (isInitialized == true) {
                  ...
                  public void onResponse(Response response) {
                    if (response.getStatusCode() >= ENPushConstants.REQUEST_SUCCESS_200
                      ...
                      try {
                        ...
                        String userIdFromResponse = map.getString(ENPushConstants.USER_ID);
                        ...
                        if (ENPushUtils.validateString(userId) && !(userId.equals(userIdFromResponse))) {
                          hasChange = true;
                        }
                        ...
                        if (hasChange) {
                          ...
                          ENPushUtils
                                  .storeContentInSharedPreferences(
                                          ...
                                          ENPushConstants.DEVICE_ID, deviceId);
                        ...
                        ENException.logException(this.getClass().getName(), "registerWithUserId", e);
      ...
      registerResponseListener.onFailure(new ENPushException("ENPush:verifyDeviceRegistrationWithUserId() - Error while registration -. Not initialized ENPush"));
    }
  ...
  private void updateTokenCallback(String deviceToken, String userId) {
    ENPushUrlBuilder builder = new ENPushUrlBuilder(instanceId, destinationId);
    JSONObject data = buildDevice(userId);
    ...
    if (isNewRegistration) {
                ...
                public void onResponse(Response response) {
                  ...
                  try {
                    ...
                    ENPushUtils
                            .storeContentInSharedPreferences(
                                    appContext, instanceId + destinationId,
                                    ENPushConstants.DEVICE_ID, retDeviceId);
      ...
      ServiceCall call =
              this.getServiceImplementation().putData(path, data, new ServiceCallback() {
                @Override
                public void onResponse(Response response) {
  ...
  private JSONObject buildDevice(String userId) {
    ...
    try {
      device.put(ENPushConstants.DEVICE_ID, regId);
      device.put(ENPushConstants.TOKEN, deviceToken);
      device.put(ENPushConstants.PLATFORM, "G");
      if (ENPushUtils.validateString(userId)) {
        device.put(ENPushConstants.USER_ID, userId);
      }
      if (registerOptions()) {
